// MUST Resource Room Notification Service v1.1.3
// Automatic reminder rule: only records marked estimated=true + estimateStage="purchasing".
// Plain estimates are never auto-reminded.
// /auto/test validates Service Account + Firestore + LINE without touching reminder state.
  ...
  async fetch(req,env,ctx){
    ...
    try{
      ...
      if((url.pathname==="/"||url.pathname==="/health")&&req.method==="GET"){
        return json({ok:true,service:"MUST Resource Room Notification Service",version:"1.1.3",scheduledReminder:true,automaticRule:"purchasing-only"},200,cors);
      }
  ...
  async scheduled(event,env,ctx){
    ctx.waitUntil(
      runAutomaticReminders(env,{source:"cron",scheduledTime:event.scheduledTime})
        .catch(e=>console.error("automatic reminder failed",e?.message||e))
    );
  }
};
...
function projectIdForServer(env){
  ...
  if(env.FIREBASE_SERVICE_ACCOUNT_JSON){
    try{return String(JSON.parse(String(env.FIREBASE_SERVICE_ACCOUNT_JSON)).project_id||"")}catch{}
  }
  fail("缺少 Firebase Project ID。",500);
}

async function serviceAccountAccessToken(env){
  const c=serviceAccountConfig(env);
...
async function importPrivateKey(pem){
  ...
  return crypto.subtle.importKey("pkcs8",base64ToBytes(b64),{name:"RSASSA-PKCS1-v1_5",hash:"SHA-256"},false,["sign"]);
}
...
async function automaticTest(identity,env,cors){
  ...
  if(!line?.groupId) fail("尚未綁定 LINE 群組。",409);
  const c=serviceAccountConfig(env);
  ...
  const enabledPlans=settingsDocs.filter(d=>{
    const s=firestoreDocumentToObject(d);
    return s.enabled===true&&s.lineEnabled!==false;
  }).length;
  const now=new Date();
  const text=[
    "【資源教室經費自動提醒－測試成功】",
    "✅ LINE 群組：正常",
    "✅ Firebase Service Account：正常",
    "✅ Firestore 讀取：正常",
    `✅ 已啟用提醒的計畫：${enabledPlans} 個`,
    "",
    "正式自動提醒目前只追蹤「請購中」紀錄；單純預估不會通知。",
    `測試時間：${formatTaipei(now)}`
  ].join("\n");
...
async function pushLine(to,text,env){
  ...
  if(!r.ok){
    ...
    fail(`LINE 發送失敗（${r.status}）${t?"："+t.slice(0,300):""}`,502);
  }
}
...
async function lineWebhook(req,env,ctx,cors){
  ...
  let body={};
  try{body=JSON.parse(raw||"{}")}catch{return json({ok:false,error:"Webhook JSON 格式錯誤。"},400,cors)}
  const groups=[];
  for(const e of Array.isArray(body.events)?body.events:[]){
    if(e?.source?.type==="group"&&e.source.groupId) groups.push({groupId:String(e.source.groupId),eventType:String(e.type||"unknown")});
  }
  if(groups.length){
    const job=saveLineGroup(groups[groups.length-1],env).catch(e=>console.error("saveLineGroup failed",e?.message||e));
...
async function verifyLineSignature(body,signature,secret){
  ...
  return arrayBufferToBase64(mac)===String(signature||"");
}
...
async function runAutomaticReminders(env,meta={}){
  ...
  const now=new Date();
  const settingsDocs=await firestoreListCollection(c.projectId,token,"reminderSettings");
  let plansChecked=0,plansSent=0,remindersSent=0;

  for(const sd of settingsDocs){
    const planId=docIdFromName(sd.name);
    const s=firestoreDocumentToObject(sd);
    if(!s.enabled||s.lineEnabled===false) continue;
    plansChecked++;

    const rows=await firestoreRunQuery(c.projectId,token,{
      from:[{collectionId:"expenseRecords"}],
      where:{fieldFilter:{field:{fieldPath:"planId"},op:"EQUAL",value:{stringValue:planId}}}
    });
    const records=rows
      .filter(x=>x.document)
      .map(x=>({id:docIdFromName(x.document.name),...firestoreDocumentToObject(x.document)}))
      .filter(r=>r.deleted!==true);

    const due=[];
    for(const r of records){
      const reason=classifyPurchasingRecord(r,s,now);
      if(!reason) continue;
      const stateId=stateDocId(planId,r.id,"purchasing");
      const state=await getReminderState(c.projectId,token,stateId);
      if(shouldSendAgain(state,s,now)) due.push({record:r,reason,stateId});
    }
    ...
    for(const x of due){
      await writeReminderState(c.projectId,token,x.stateId,{
        planId,
        recordId:x.record.id,
        reason:"purchasing",
        reasonLabel:"請購中待核銷",
        lastSentAt:sentAt,
        lastEligibleAt:x.reason.eligibleAt.toISOString(),
        updatedAt:sentAt
      });
    }
    plansSent++;
...
function classifyPurchasingRecord(r,s,now){
  ...
  if(r?.estimated!==true) return null;
  if(String(r?.estimateStage||"")!=="purchasing") return null;
  const base=timestampDate(r.updatedAt||r.createdAt);
  if(!base) return null;
  const afterDays=numberOr(s.purchasingAfterDays,s.estimatedAfterDays??14);
  const time=validTime(s.purchasingTime)?s.purchasingTime:(validTime(s.estimatedTime)?s.estimatedTime:DEFAULT_TIME);
  const enabled=s.purchasingEnabled!==undefined?s.purchasingEnabled:s.estimatedEnabled;
  if(enabled===false) return null;
  const eligibleAt=eligibilityDate(base,afterDays,time);
  return now>=eligibleAt?{eligibleAt}:null;
}

function buildAutomaticMessage(s,due){
  const map=new Map();
  for(const x of due){
    const key=x.record.ownerEmail||x.record.ownerName||"未填老師";
    const row=map.get(key)||{name:x.record.ownerName||x.record.ownerEmail||"未填老師",count:0};
    row.count++;
    map.set(key,row);
  }
...
function shouldSendAgain(state,s,now){
  if(!state?.lastSentAt) return true;
  const last=timestampDate(state.lastSentAt);
  if(!last) return true;
  return now.getTime()>=last.getTime()+Math.max(1,numberOr(s.repeatEveryDays,1))*86400000;
}
...
function eligibilityDate(base,days,time){
  const p={};
  for(const x of new Intl.DateTimeFormat("en-CA",{timeZone:"Asia/Taipei",year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:false}).formatToParts(base)){
    if(x.type!=="literal") p[x.type]=x.value;
  }
  const [hh,mm]=(validTime(time)?time:DEFAULT_TIME).split(":").map(Number);
  return new Date(Date.UTC(+p.year,+p.month-1,+p.day+Number(days||0),hh-8,mm,0,0));
}
...
async function firestoreListCollection(projectId,token,id){
  let page="",docs=[];
  do{
    const u=new URL(`${firestoreBase(projectId)}/${encodeURIComponent(id)}`);
    u.searchParams.set("pageSize","300");
    if(page) u.searchParams.set("pageToken",page);
    const r=await fetch(u,{headers:{Authorization:`Bearer ${token}`}});
    if(!r.ok) throw new Error(`讀取 ${id} 失敗（${r.status}）`);
    const d=await r.json();
...
async function firestoreRunQuery(projectId,token,q){
  const r=await fetch(`${firestoreBase(projectId)}:runQuery`,{
    method:"POST",
    headers:{Authorization:`Bearer ${token}`,"content-type":"application/json"},
    body:JSON.stringify({structuredQuery:q})
  });
  if(!r.ok) throw new Error(`Firestore 查詢失敗（${r.status}）`);
  return r.json();
}
async function getReminderState(projectId,token,id){
  const r=await fetch(firestoreDocUrl(projectId,`reminderAutoState/${id}`),{headers:{Authorization:`Bearer ${token}`}});
  if(r.status===404) return null;
  if(!r.ok) throw new Error(`讀取提醒狀態失敗（${r.status}）`);
  return firestoreDocumentToObject(await r.json());
}
async function writeReminderState(projectId,token,id,data){
  const r=await fetch(firestoreDocUrl(projectId,`reminderAutoState/${id}`),{
    method:"PATCH",
    headers:{Authorization:`Bearer ${token}`,"content-type":"application/json"},
    body:JSON.stringify({fields:objectToFirestoreFields(data)})
  });
  if(!r.ok) throw new Error(`寫入提醒狀態失敗（${r.status}）`);
}
...
function firestoreValueToJs(v){
  ...
  if("mapValue"in v){const o={};for(const[k,x]of Object.entries(v.mapValue.fields||{}))o[k]=firestoreValueToJs(x);return o}
  return null;
}
function objectToFirestoreFields(o){
  const out={};
  for(const[k,v]of Object.entries(o)){
    out[k]=typeof v==="boolean"?{booleanValue:v}:typeof v==="number"?(Number.isInteger(v)?{integerValue:String(v)}:{doubleValue:v}):{stringValue:String(v)};
  }
  return out;
}
...
function githubConfig(env){
  ...
  if(!owner||!repo||!token) fail("通知服務尚未設定私密 GitHub Repository 權限。",500);
  return {owner,repo,token,branch:env.BUDGET_GITHUB_BRANCH||"main"};
}
async function gh(cfg,path,init={}){
  const r=await fetch(`https://api.github.com${path}`,{
    ...init,
    headers:{Accept:"application/vnd.github+json",Authorization:`Bearer ${cfg.token}`,"X-GitHub-Api-Version":"2022-11-28","User-Agent":"MUST-Resource-Notify-Service",...(init.body?{"content-type":"application/json"}:{}),...(init.headers||{})}
  });
  const d=await r.json().catch(()=>({}));
  if(!r.ok) fail(d.message||`GitHub API 錯誤（${r.status}）`,502);
  return d;
}
async function ghOptional(cfg,path){
  const r=await fetch(`https://api.github.com${path}`,{headers:{Accept:"application/vnd.github+json",Authorization:`Bearer ${cfg.token}`,"X-GitHub-Api-Version":"2022-11-28","User-Agent":"MUST-Resource-Notify-Service"}});
  if(r.status===404) return null;
  const d=await r.json().catch(()=>({}));
  if(!r.ok) fail(d.message||`GitHub API 錯誤（${r.status}）`,502);
  return d;
...
async function saveLineGroup(group,env){
  ...
  let previous={};
  if(existing?.content){try{previous=JSON.parse(new TextDecoder().decode(base64ToBytes(String(existing.content).replace(/\s+/g,""))))}catch{}}
  ...
  const body={message:"link LINE notification group",branch:cfg.branch,content:bytesToBase64(new TextEncoder().encode(JSON.stringify(payload,null,2)))};
...
async function loadLineConfig(env){
  ...
  return JSON.parse(new TextDecoder().decode(base64ToBytes(String(d.content).replace(/\s+/g,""))));
}
