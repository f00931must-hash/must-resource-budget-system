// MUST Free Upload Service v0.5.0
  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if ((url.pathname === "/" || url.pathname === "/health") && req.method === "GET") {
        return json({ ok: true, service: "MUST Free Upload Service", version: "0.5.0" }, 200, cors);
      }
      ...
      if (url.pathname === "/stats" && req.method === "GET") return stats(url, id, env, cors);
...
async function authorize(id, env) {
  ...
  if (id.project === env.BUDGET_FIREBASE_PROJECT_ID) {
    ...
    const d = await fsGet(`${base}/${encodeURIComponent(id.email)}`, id.token);
    ...
    if (enabled !== true || !["manager", "user"].includes(role)) fail("此帳號沒有經費附件存取權限。", 403);
    return;
  }
