// MUST Resource Room Notification Service v0.1.2
// LINE Messaging API webhook + manager-only test notification endpoint.
// Safe diagnostics: logs GitHub HTTP status/message/repo/path, never token values.
  ...
  async fetch(req, env, ctx) {
    ...
    try {
      ...
      if ((url.pathname === "/" || url.pathname === "/health") && req.method === "GET") {
        return json({ ok: true, service: "MUST Resource Room Notification Service", version: "0.1.2" }, 200, cors);
      }
...
function githubConfig(env) {
  ...
  if (!owner || !repo || !token) {
    const missing = [!owner ? "BUDGET_GITHUB_OWNER" : "", !repo ? "BUDGET_GITHUB_REPO" : "", !token ? "BUDGET_GITHUB_TOKEN" : ""].filter(Boolean);
    fail(`通知服務缺少 GitHub 設定：${missing.join(", ")}`, 500);
  }
  return { owner, repo, token, branch: env.BUDGET_GITHUB_BRANCH || "main" };
}

function safeGitHubErrorLog(label, cfg, path, response, data) {
  console.error(label, {
    githubStatus: response.status,
    githubMessage: String(data?.message || "Unknown GitHub error").slice(0, 300),
    githubRepo: `${cfg.owner}/${cfg.repo}`,
    githubPath: String(path).slice(0, 500),
...
async function gh(cfg, path, init = {}) {
  ...
  if (!r.ok) {
    safeGitHubErrorLog("notify-github-api-error", cfg, path, r, d);
    fail(`GitHub API ${r.status}: ${d.message || "Unknown error"}`, r.status === 404 ? 404 : 502);
  }
  return d;
}
...
async function ghOptional(cfg, path, init = {}) {
  ...
  if (!r.ok) {
    safeGitHubErrorLog("notify-github-api-error", cfg, path, r, d);
    fail(`GitHub API ${r.status}: ${d.message || "Unknown error"}`, 502);
  }
  return d;
}
