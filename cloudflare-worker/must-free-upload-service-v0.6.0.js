// MUST Free Upload Service v0.6.0
// - Keeps existing activity / announcement / portal uploads unchanged
// - Budget attachments remain private in must-resource-private-assets
// - Budget attachment max size: 15 MB
// - Budget attachment authorization no longer reads Budget Firestore on every request
// - Budget access list is synced from Portal admin and stored in the private GitHub repo
// - Firebase ID token + private access list are both required
  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if ((url.pathname === "/" || url.pathname === "/health") && req.method === "GET") {
        return json({ ok: true, service: "MUST Free Upload Service", version: "0.6.0" }, 200, cors);
      }

      const id = await authenticate(req, env);
      await authorize(id, env);

      if (url.pathname === "/budget-access-sync" && req.method === "POST") return budgetAccessSync(req, id, env, cors);
