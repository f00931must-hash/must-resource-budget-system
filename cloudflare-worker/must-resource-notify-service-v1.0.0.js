// MUST Resource Room Notification Service v1.0.0
// Production LINE notification worker.
// - Verifies LINE webhook signatures.
// - Stores the linked LINE group ID in the private GitHub repo.
// - Exposes manager-only LINE status / test / send endpoints.
// - Never logs secrets or group IDs.
  ...
  async fetch(req, env, ctx) {
    ...
    try {
      ...
      if ((url.pathname === "/" || url.pathname === "/health") && req.method === "GET") {
        return json({ ok: true, service: "MUST Resource Room Notification Service", version: "1.0.0" }, 200, cors);
      }

      // LINE webhook is authenticated with X-Line-Signature, not Firebase.
      if (url.pathname === "/line/webhook" && req.method === "POST") {
        return lineWebhook(req, env, ctx, cors);
      }

      // All management endpoints require Budget Firebase login + manager role.
...
async function lineWebhook(req, env, ctx, cors) {
  ...
  for (const event of Array.isArray(body.events) ? body.events : []) {
    ...
    if (source?.type === "group" && source.groupId) {
      groups.push({ groupId: String(source.groupId), eventType: String(event.type || "unknown") });
    }
  }

  if (groups.length) {
    const latest = groups[groups.length - 1];
    const job = saveLineGroup(latest, env).catch(err => console.error("saveLineGroup failed", err?.message || err));
...
async function lineTest(req, id, env, cors) {
  return sendManagerLine(req, id, env, cors, "【資源教室經費提醒－測試】\nLINE 提醒系統已成功連線。");
}

async function lineSend(req, id, env, cors) {
  return sendManagerLine(req, id, env, cors, "【資源教室經費提醒】\n目前有經費資料待處理，請進入經費系統確認。");
}

async function sendManagerLine(req, id, env, cors, fallbackText) {
  const cfg = await loadLineConfig(env);
  if (!cfg?.groupId) fail("尚未綁定 LINE 群組。", 409);
  const body = await req.json().catch(() => ({}));
  const text = String(body.text || fallbackText).trim().slice(0, 4500);
