// MUST Free Upload Service v0.5.4
...
// - Budget permission check: 2-minute in-memory cache + automatic retry on Firestore 429

const JSON_HEADERS = { "content-type": "application/json; charset=utf-8" };
const BUDGET_AUTH_CACHE_TTL_MS = 2 * 60 * 1000;
const budgetAuthCache = new Map();
  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if ((url.pathname === "/" || url.pathname === "/health") && req.method === "GET") {
        return json({ ok: true, service: "MUST Free Upload Service", version: "0.5.4" }, 200, cors);
      }
...
async function authorize(id, env) {
  ...
  if (id.project === env.BUDGET_FIREBASE_PROJECT_ID) {
    const cacheKey = `${id.project}:${id.email}`;
    const now = Date.now();
    const cached = budgetAuthCache.get(cacheKey);
    ...
    if (cached) budgetAuthCache.delete(cacheKey);
    ...
    budgetAuthCache.set(cacheKey, {
      enabled,
      role,
      expiresAt: now + BUDGET_AUTH_CACHE_TTL_MS
    });

    if (enabled !== true) fail("此帳號目前未啟用經費附件權限。", 403);
    if (role === "assistant") fail("小幫手不開放經費附件存取。", 403);
    return;
  }
