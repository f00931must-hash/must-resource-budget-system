// MUST Resource Room Notification Service v0.1.0
// LINE Messaging API webhook + manager-only test notification endpoint.
// Kept separate from the attachment/upload Worker so notification failures cannot affect vouchers.
  ...
  async fetch(req, env, ctx) {
    ...
    try {
      ...
      if ((url.pathname === "/" || url.pathname === "/health") && req.method === "GET") {
        return json({ ok: true, service: "MUST Resource Room Notification Service", version: "0.1.0" }, 200, cors);
      }

      // LINE calls this endpoint without Firebase Authorization.
      // Authentication is the X-Line-Signature HMAC-SHA256 signature.
      if (url.pathname === "/line/webhook" && req.method === "POST") {
        return lineWebhook(req, env, ctx, cors);
      }

      // Everything below requires a valid Budget Firebase login and manager role.
      ...
      if (url.pathname === "/line/test" && req.method === "POST") {
        return lineTest(req, id, env, cors);
      }
...
async function authenticateBudget(req, env) {
  ...
  if (String(p.aud || "") !== String(env.BUDGET_FIREBASE_PROJECT_ID || "")) {
...
async function lineWebhook(req, env, ctx, cors) {
  ...
  // LINE requires a fast 2xx response. Do not send a bot reply here.
...
async function saveLineGroup(group, env) {
  ...
  if (existing?.content) {
    try {
      previous = JSON.parse(new TextDecoder().decode(base64ToBytes(String(existing.content).replace(/\s+/g, ""))));
    } catch {}
  }

  // Avoid making a GitHub commit for every normal message once the same group is known.
  ...
  await gh(cfg, `/repos/${cfg.owner}/${cfg.repo}/contents/${enc(LINE_CONFIG_PATH)}`, {
    method: "PUT",
