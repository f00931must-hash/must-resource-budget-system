...
let isManager = false;
let cache = { planId:"", categories:[], records:[] };
...
async function refreshAndPatch(){
  ...
  if(!planId) return;
  try{
    const [catSnap, recSnap] = await Promise.all([
    ...
    console.warn("budget v1.2.1 patch refresh failed", err);
  }
}
...
function patchRecordCards(){
  ...
  document.querySelectorAll("#recordList .record-card").forEach(card=>{
    ...
    if(waived){
      ...
      if(statusArea && !statusArea.querySelector(".voucher-waived-badge")){
    ...
    if(!isManager || r.estimated===true || r.reviewStatus==="approved" || r.reviewed===true || r.locked===true) return;
...
function patchTodoCounts(){
  if(!isManager) return;
  const missingVoucher=cache.records.filter(r=>r.estimated!==true && !(r.voucherUrl||r.folderUrl) && r.voucherNotRequired!==true && r.voucherRequirementWaived!==true).length;
  const missingConfirm=cache.records.filter(r=>r.estimated!==true && r.voucherNotRequired!==true && r.voucherRequirementWaived!==true && r.amountConfirmed!==true && r.amountManuallyConfirmed!==true).length;
