// Budget admin unified controller v1.7.0
// Performance-safe manager controller.
// Keeps manager waiver/review behavior while avoiding Firestore re-queries caused by its own DOM patches.
...
let timer=null, refreshInFlight=false, refreshQueued=false;
...
function cardSignature(){
  return [...document.querySelectorAll("#recordList .record-card")].map(recordId).filter(Boolean).join("|");
}
function ensureBadge(area,text,cls="danger-badge"){
  if(!area) return;
  if([...area.querySelectorAll(".status")].some(x=>x.textContent.trim()===text)) return;
  const b=document.createElement("span"); b.className=`status ${cls}`; b.textContent=text; area.appendChild(b);
}
...
function placeBeforeApprove(actions,button){
  ...
  if(approve) approve.insertAdjacentElement("beforebegin",button); else actions.appendChild(button);
}
...
function patchCards(){
  ...
  document.querySelectorAll("#recordList .record-card").forEach(card=>{
    ...
    const actions=card.querySelector(".record-actions"), status=card.children?.[2];
    if(!actions) return;
    const confirmed=managerConfirmed(r), voucher=hasVoucher(r);

    if(!voucher && !confirmed){
      removeBadge(status,"免附單據"); ensureBadge(status,"缺單據");
      if(r.amountConfirmed!==true && r.amountManuallyConfirmed!==true) ensureBadge(status,"未確認金額");
      actions.querySelectorAll("[data-manager-revoke-waiver]").forEach(x=>x.remove());
      if(!actions.querySelector("[data-manager-waive-voucher]")){
        const b=document.createElement("button"); b.className="link-btn"; b.dataset.managerWaiveVoucher=id; b.textContent="免附單據";
        b.title="管理員二次確認此筆確實不需要核銷附件"; placeBeforeApprove(actions,b);
      }
    }else if(confirmed){
      removeBadge(status,"缺單據"); removeBadge(status,"未確認金額"); ensureBadge(status,"免附單據","done");
      actions.querySelectorAll("[data-manager-waive-voucher]").forEach(x=>x.remove());
      if(!actions.querySelector("[data-manager-revoke-waiver]")){
        const b=document.createElement("button"); b.className="link-btn"; b.dataset.managerRevokeWaiver=id; b.textContent="撤銷免附";
        b.title="撤銷管理員的免附單據確認"; placeBeforeApprove(actions,b);
      }
    }else{
      actions.querySelectorAll("[data-manager-waive-voucher],[data-manager-revoke-waiver]").forEach(x=>x.remove());
    }
  });
}
...
function patchVisuals(){ patchCards(); patchTodoCounts(); }

async function refresh(){
  if(!manager || !db) return;
  if(refreshInFlight){ refreshQueued=true; return; }
  const planId=document.getElementById("planSelect")?.value||"";
  if(!planId){ records=[]; patchVisuals(); return; }
  ...
  try{
    ...
    if(refreshQueued){ refreshQueued=false; schedule(180); }
  }
}
...
async function verifyManagerAndRecord(id){
  ...
  const [userSnap,recordSnap]=await Promise.all([getDoc(doc(aDb,"users",email)),getDoc(doc(aDb,"expenseRecords",id))]);
  ...
  return {db:aDb,email,r:recordSnap.data()};
}
...
async function confirmWaiver(id){
  try{
    ...
    await updateDoc(doc(db,"expenseRecords",id),{voucherWaiverManagerConfirmed:true,voucherWaiverManagerConfirmedBy:email,voucherWaiverManagerConfirmedAt:serverTimestamp(),voucherNotRequired:true,voucherRequirementWaived:true,amountConfirmed:true,amountConfirmedByManagerWaiver:true,updatedAt:serverTimestamp(),updatedBy:email});
...
async function revokeWaiver(id){
  try{
    const {db,email,r}=await verifyManagerAndRecord(id); if(!managerConfirmed(r)) return;
    if(!confirm(`確定撤銷「${r.purpose||"此筆"}」的免附單據確認？\n\n撤銷後會恢復為需要核銷單據。`)) return;
    await updateDoc(doc(db,"expenseRecords",id),{voucherWaiverManagerConfirmed:false,voucherWaiverManagerConfirmedBy:"",voucherWaiverManagerConfirmedAt:null,voucherNotRequired:false,voucherRequirementWaived:false,amountConfirmed:false,amountConfirmedByManagerWaiver:false,updatedAt:serverTimestamp(),updatedBy:email});
...
async function approveWaived(id){
  try{
    const {db,email,r}=await verifyManagerAndRecord(id); if(!managerConfirmed(r)) return false;
    if(!confirm(`確認「${r.purpose||"此筆"}」核對完成？\n${money(r.amount)}\n\n此筆已由管理員確認免附單據；完成後會鎖定。`)) return true;
    await updateDoc(doc(db,"expenseRecords",id),{reviewStatus:"approved",reviewed:true,locked:true,reviewedAt:serverTimestamp(),reviewedBy:email,updatedAt:serverTimestamp(),updatedBy:email});
    await addDoc(collection(db,"auditLogs"),{type:"expense-review",targetId:id,planId:r.planId||"",action:"approve-manager-waived-voucher",actorEmail:email,createdAt:serverTimestamp()});
...
document.addEventListener("click",async e=>{
  ...
  if(approve && manager){
    const id=approve.dataset.approveRecord||"", r=records.find(x=>x.id===id);
    if(r && !hasVoucher(r)){
      e.preventDefault(); e.stopImmediatePropagation();
      if(managerConfirmed(r)) await approveWaived(id);
      else alert(r.voucherNotRequiredRequested===true?"此筆老師已申請『此單無需附上單據』。請先按『免附單據』由管理員二次確認，再核對完成。":"此筆缺少核銷單據。若確實不需附件，請先按『免附單據』確認。");
    }
  }
},true);
...
async function init(){
  ...
  onAuthStateChanged(auth,async user=>{
    ...
    try{
      ...
      if(manager) schedule(80);
    }catch(err){ console.warn(err); }
  });

  document.getElementById("planSelect")?.addEventListener("change",()=>schedule(120));
  ["filterIssue","filterReview","filterCategory","filterSemester"].forEach(id=>document.getElementById(id)?.addEventListener("input",()=>patchVisuals()));
  ...
  if(list){
    ...
    new MutationObserver(()=>{
      ...
      if(sig!==lastCardSignature){ lastCardSignature=sig; schedule(140); }
      else setTimeout(patchVisuals,0);
